...
expected_target_hash() {
  case "$1" in
    backend/app/rag/document_metadata_repository.py)
      printf '%s' 'b53a1d9daa0fa2ad31e777b9f8ab15db29237650b201b8d5bd5f63af387d9e91'
      ;;
    backend/app/rag/document_metadata_schema.py)
      printf '%s' 'daa96ad776279d01230c8e18528567909e36779617528effa175aef069cbcf26'
      ;;
    *) return 1 ;;
  esac
}
...
echo "[1/5] 文書削除の回帰テストを実行しています"
(
  cd "${SOURCE_ROOT}/backend"
  PYTHONPATH=. "${TARGET_ROOT}/backend/.venv/bin/python" -m pytest -q     tests/test_document_metadata_feature.py     -k 'registered_document_delete'
)

echo "[2/5] 稼働中ファイルをバックアップしています: ${BACKUP_ROOT}"
...
echo "失敗文書の削除エラー修正をデプロイしました"
echo "バックアップ: ${BACKUP_ROOT}"
echo "ロールバック: sudo bash ${SOURCE_ROOT}/scripts/rollback-document-delete-fk-fix.sh ${BACKUP_ROOT}"
