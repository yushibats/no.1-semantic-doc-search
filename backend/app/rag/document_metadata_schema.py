...
TABLE_STATEMENTS: dict[str, str] = {
    ...
    """,
    "SDS_PAGE_CLASSIFICATIONS": """
        CREATE TABLE SDS_PAGE_CLASSIFICATIONS (
            DOCUMENT_REVISION_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENT_REVISIONS(REVISION_ID) ON DELETE CASCADE,
            PAGE_NUMBER NUMBER NOT NULL CHECK (PAGE_NUMBER >= 1),
            DOCUMENT_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENTS(DOCUMENT_ID) ON DELETE CASCADE,
            CONTENT_KIND VARCHAR2(24) DEFAULT 'OTHER' NOT NULL
                CHECK (CONTENT_KIND IN ('FLOOR_PLAN', 'SITE_PLAN', 'ELEVATION',
                                        'AREA_TABLE', 'PERSPECTIVE', 'PHOTO', 'OTHER')),
            PHASE VARCHAR2(16) DEFAULT 'UNKNOWN' NOT NULL
                CHECK (PHASE IN ('EXISTING', 'PROPOSED', 'COMPLETED', 'UNKNOWN')),
            FLOOR_CODE VARCHAR2(32),
            PLAN_VARIANT VARCHAR2(64),
    ...
    """,
    "SDS_DOCUMENT_SET_FACTS": """
        CREATE TABLE SDS_DOCUMENT_SET_FACTS (
            ...
            FACT_CODE VARCHAR2(32) NOT NULL
                CHECK (FACT_CODE IN ('BUILDING_TYPE', 'STRUCTURE', 'USE', 'AREA_TYPE',
                                     'AREA_VALUE', 'LAYOUT')),
            PHASE VARCHAR2(16) DEFAULT 'COMMON' NOT NULL
                CHECK (PHASE IN ('COMMON', 'EXISTING', 'PROPOSED', 'COMPLETED')),
            VALUE_TEXT VARCHAR2(400),
            VALUE_NUMBER NUMBER,
            UNIT VARCHAR2(32),
            SOURCE VARCHAR2(16) DEFAULT 'RULE' NOT NULL
                CHECK (SOURCE IN ('RULE', 'VLM', 'USER', 'MIGRATION')),
            ...
            CREATED_AT TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
            UPDATED_AT TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
            UNIQUE (DOCUMENT_SET_ID, FACT_CODE, PHASE),
    ...
    """,
    "SDS_DOCUMENT_SET_NUMERIC_FACTS": """
        CREATE TABLE SDS_DOCUMENT_SET_NUMERIC_FACTS (
            NUMERIC_FACT_ID VARCHAR2(64) PRIMARY KEY,
            DOCUMENT_SET_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENT_SETS(DOCUMENT_SET_ID) ON DELETE CASCADE,
            FACT_CODE VARCHAR2(32) NOT NULL
                CHECK (FACT_CODE IN ('FLOOR_COUNT', 'BUILDING_AGE')),
            PHASE VARCHAR2(16) DEFAULT 'COMMON' NOT NULL
                CHECK (PHASE IN ('COMMON', 'EXISTING', 'PROPOSED', 'COMPLETED')),
            VALUE_NUMBER NUMBER NOT NULL CHECK (VALUE_NUMBER >= 0),
            UNIT VARCHAR2(8) NOT NULL CHECK (UNIT IN ('階', '年')),
            SOURCE VARCHAR2(16) DEFAULT 'RULE' NOT NULL
                CHECK (SOURCE IN ('RULE', 'VLM', 'USER', 'MIGRATION')),
            CONFIDENCE NUMBER DEFAULT 0 NOT NULL CHECK (CONFIDENCE BETWEEN 0 AND 1),
            CONFIRMED NUMBER(1) DEFAULT 0 NOT NULL CHECK (CONFIRMED IN (0, 1)),
    ...
    """,
    "SDS_ROOM_MEASUREMENTS": """
        CREATE TABLE SDS_ROOM_MEASUREMENTS (
            MEASUREMENT_ID VARCHAR2(64) PRIMARY KEY,
            DOCUMENT_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENTS(DOCUMENT_ID) ON DELETE CASCADE,
            DOCUMENT_SET_ID VARCHAR2(64)
                REFERENCES SDS_DOCUMENT_SETS(DOCUMENT_SET_ID) ON DELETE SET NULL,
            DOCUMENT_REVISION_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENT_REVISIONS(REVISION_ID) ON DELETE CASCADE,
            PAGE_NUMBER NUMBER NOT NULL CHECK (PAGE_NUMBER >= 1),
            PHASE VARCHAR2(16) DEFAULT 'COMMON' NOT NULL
            ...
            AREA_M2 NUMBER CHECK (AREA_M2 > 0),
            TATAMI_EQUIVALENT NUMBER CHECK (TATAMI_EQUIVALENT > 0),
            BASIS VARCHAR2(32) NOT NULL
                CHECK (BASIS IN ('PRINTED_TATAMI', 'PRINTED_AREA',
                                 'VERIFIED_RECTANGLE', 'ESTIMATED_L_SHAPE')),
            SOURCE VARCHAR2(16) DEFAULT 'RULE' NOT NULL
                CHECK (SOURCE IN ('RULE', 'VLM', 'USER', 'MIGRATION')),
            CONFIDENCE NUMBER DEFAULT 0 NOT NULL CHECK (CONFIDENCE BETWEEN 0 AND 1),
            CONFIRMED NUMBER(1) DEFAULT 0 NOT NULL CHECK (CONFIRMED IN (0, 1)),
            USER_LOCKED NUMBER(1) DEFAULT 0 NOT NULL CHECK (USER_LOCKED IN (0, 1)),
            REVIEW_STATUS VARCHAR2(24) DEFAULT 'REVIEW_REQUIRED' NOT NULL
                CHECK (REVIEW_STATUS IN ('AUTO_VERIFIED', 'REVIEW_REQUIRED',
                                         'USER_CONFIRMED', 'REJECTED')),
            SEARCHABLE NUMBER(1) DEFAULT 1 NOT NULL CHECK (SEARCHABLE IN (0, 1)),
            EVIDENCE_JSON CLOB CHECK (EVIDENCE_JSON IS JSON),
            CREATED_AT TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
            UPDATED_AT TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
            CHECK (AREA_M2 IS NOT NULL OR TATAMI_EQUIVALENT IS NOT NULL),
            UNIQUE (DOCUMENT_REVISION_ID, PAGE_NUMBER, PHASE, ROOM_NAME, BASIS)
        )
    """,
    "SDS_COMPARISON_PAIRS": """
        CREATE TABLE SDS_COMPARISON_PAIRS (
            PAIR_ID VARCHAR2(64) PRIMARY KEY,
            DOCUMENT_SET_ID VARCHAR2(64) NOT NULL UNIQUE
                REFERENCES SDS_DOCUMENT_SETS(DOCUMENT_SET_ID) ON DELETE CASCADE,
            FLOOR_CODE VARCHAR2(32),
            PLAN_VARIANT VARCHAR2(64),
            BEFORE_DOCUMENT_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENTS(DOCUMENT_ID) ON DELETE CASCADE,
            BEFORE_REVISION_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENT_REVISIONS(REVISION_ID) ON DELETE CASCADE,
            BEFORE_PAGE_NUMBER NUMBER NOT NULL CHECK (BEFORE_PAGE_NUMBER >= 1),
            AFTER_DOCUMENT_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENTS(DOCUMENT_ID) ON DELETE CASCADE,
            AFTER_REVISION_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENT_REVISIONS(REVISION_ID) ON DELETE CASCADE,
            AFTER_PAGE_NUMBER NUMBER NOT NULL CHECK (AFTER_PAGE_NUMBER >= 1),
            SOURCE VARCHAR2(8) DEFAULT 'USER' NOT NULL CHECK (SOURCE IN ('AUTO', 'USER')),
            USER_LOCKED NUMBER(1) DEFAULT 1 NOT NULL CHECK (USER_LOCKED IN (0, 1)),
            UPDATED_BY_HASH CHAR(64),
            CREATED_AT TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL,
            UPDATED_AT TIMESTAMP DEFAULT SYSTIMESTAMP NOT NULL
        )
    """,
    "SDS_COMPARISON_ANALYSES": """
        CREATE TABLE SDS_COMPARISON_ANALYSES (
            ANALYSIS_ID VARCHAR2(64) PRIMARY KEY,
            DOCUMENT_SET_ID VARCHAR2(64) NOT NULL
                REFERENCES SDS_DOCUMENT_SETS(DOCUMENT_SET_ID) ON DELETE CASCADE,
            PAIR_ID VARCHAR2(64) REFERENCES SDS_COMPARISON_PAIRS(PAIR_ID) ON DELETE SET NULL,
...
INDEX_STATEMENTS: dict[str, str] = {
    ...
    """,
    "SDS_PAGECLASS_DOC_IDX": """
        CREATE INDEX SDS_PAGECLASS_DOC_IDX ON SDS_PAGE_CLASSIFICATIONS (
            DOCUMENT_ID, PHASE, CONTENT_KIND, FLOOR_CODE
        )
    """,
    "SDS_SETFACT_FILTER_IDX": """
        CREATE INDEX SDS_SETFACT_FILTER_IDX ON SDS_DOCUMENT_SET_FACTS (
            FACT_CODE, PHASE, CONFIRMED, VALUE_TEXT, DOCUMENT_SET_ID
        )
    """,
    ...
    """,
    "SDS_SETNUM_FILTER_IDX": """
        CREATE INDEX SDS_SETNUM_FILTER_IDX ON SDS_DOCUMENT_SET_NUMERIC_FACTS (
            FACT_CODE, CONFIRMED, VALUE_NUMBER, DOCUMENT_SET_ID
        )
    """,
    "SDS_ROOMMEASURE_FILTER_IDX": """
        CREATE INDEX SDS_ROOMMEASURE_FILTER_IDX ON SDS_ROOM_MEASUREMENTS (
            ROOM_NAME, PHASE, SEARCHABLE, REVIEW_STATUS,
            TATAMI_EQUIVALENT, AREA_M2, DOCUMENT_SET_ID, DOCUMENT_ID
        )
    """,
    "SDS_COMPARISON_ANALYSIS_IDX": """
        CREATE INDEX SDS_COMPARISON_ANALYSIS_IDX ON SDS_COMPARISON_ANALYSES (
            DOCUMENT_SET_ID, STATUS, UPDATED_AT
        )
    """,
}
...
ALTER_COLUMNS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        "SDS_COMPARISON_ANALYSES",
        "REQUEST_JSON",
        "ALTER TABLE SDS_COMPARISON_ANALYSES ADD REQUEST_JSON CLOB "
        "CHECK (REQUEST_JSON IS JSON)",
    ),
)
