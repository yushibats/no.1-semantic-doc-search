...
- 長方形1個で検証できる部屋はconfidence 0.80以上を目安に返す
- L字形など複数長方形で概算する部屋は、各寸法と分割根拠を読める場合、confidence 0.55以上なら返す。概算で要確認であることをevidenceへ明記する
- L字形の概算値も検索候補に使うため、完全な確信がなくても根拠のある推定は省略しない。ただし読めない寸法、縮尺からのピクセル推定、隣室や家具の寸法は使わない
- room_area_estimatesのarea_m2とtatami_equivalentはサーバー側で再計算するため、0を返す
- 面積から帖数への換算は1帖=1.62㎡とする。検索時は入力値に対して常に±10%の許容幅を使う
